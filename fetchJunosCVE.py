...
NVD_API_KEY = 'your_nvd_api_key_here'  # Replace with your actual NVD API key
...
NVD_MAX_WORKERS = 2  # Maximum number of worker threads for NVD API calls
...
def process_device(token, device, eol_data, eol_data_exists):
    ...
    if is_juniper:
        ...
        return None, is_juniper, device_version
    ...
    try:
        ...
        if findings:
            ...
            email_body = (
                ...
                f"Total devices with vulnerabilities: {len([finding for finding in findings if 'Vulnerabilities' in finding])}\n"
                f"Total devices with EOL versions: {len([finding for finding in findings if 'EOL Date' in finding])}\n"
